    ...
    >
      ...
      >
        ...
        <textarea
          ...
          class="work-textarea work-textarea-solo"
        ...
        >
          ...
          <div v-if="self.columns.filter(c => c.name == 'limits') && self.columns.filter(c => c.name == 'limits').length > 0">
            <VueDraggable
              v-model="self.limits"
                ...
                >
                  ...
                  >
                    <textarea
                      ...
                      @input="
                        update_child(limit);
                        $event.target.style.height = `${Number(column.textsize) + 11}px`;
                        $event.target.style.height = `${$event.target.scrollHeight}px`;
                      "
                      class="work-textarea no-focus"
                      style="padding: 5px 0px;"
                      :style="`font-size: ${column.textsize}px; height: ${Number(column.textsize) + 11}px;`"
                      :class="edit_mode ? 'white' : (get_column_color(column.name)) + ' lighten-4'"
                      :disabled="!edit_mode"
                    ></textarea>
                  </div>
        ...
        >
          ...
          <div v-if="self.columns.filter(c => c.name == 's460s') && self.columns.filter(c => c.name == 's460s').length > 0">
            <VueDraggable
              v-model="self.s460s"
